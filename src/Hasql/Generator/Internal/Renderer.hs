...
import Hasql.Generator.Internal.Database.Sql.Types
  ( ColumnMetadata
    ...
    FragmentMetadata (ColumnFragment, ExpressionFragment, UnknownFragment),
...
import Hasql.Generator.Internal.Renderer.Types
  ( RenderingIssue
      ( FragmentMetadataContainsUnknownFragment,
        FragmentMetadataContainsUnknownType
      ),
  )
...
toHaskell sql parameterAndResultMetadata limit moduleName functionName =
  case (parameterTypesAndConstraints, resultTypesAndConstraints) of
    (Left parameterIssues, Right _resultTypes) -> Left parameterIssues
    (Right _parameterTypes, Left resultIssues) -> Left resultIssues
    (Left parameterIssues, Left resultIssues) -> Left (parameterIssues ++ resultIssues)
    (Right parameterTypes, Right resultTypes) -> Right $ haskellOutput parameterTypes resultTypes
  where
    parameterTypesAndConstraints :: Either [RenderingIssue] [(PostgresqlType, NullabilityConstraint)]
    parameterTypesAndConstraints =
      fragmentMetadatasToTypesAndConstraints parameterAndResultMetadata.parameterMetadata

    resultTypesAndConstraints :: Either [RenderingIssue] [(PostgresqlType, NullabilityConstraint)]
    resultTypesAndConstraints =
      fragmentMetadatasToTypesAndConstraints parameterAndResultMetadata.resultMetadata
...
fragmentMetadatasToTypesAndConstraints ::
  [FragmentMetadata] ->
  Either [RenderingIssue] [(PostgresqlType, NullabilityConstraint)]
fragmentMetadatasToTypesAndConstraints fragments =
  foldl'
    toRenderingIssuesOrTypesAndConstraints
    (Right [])
    (fmap toTypeAndConstraint fragments)
  where
    toRenderingIssuesOrTypesAndConstraints ::
      Either [RenderingIssue] [(PostgresqlType, NullabilityConstraint)] ->
      Either RenderingIssue (PostgresqlType, NullabilityConstraint) ->
      Either [RenderingIssue] [(PostgresqlType, NullabilityConstraint)]
    toRenderingIssuesOrTypesAndConstraints accumulator eType = case accumulator of
      Left issues -> case eType of
        Left issue -> Left $ issue : issues
        Right _ -> Left issues
      Right haskellTypes -> case eType of
        Left issue -> Left [issue]
        Right haskellType -> Right $ haskellTypes ++ [haskellType]

    toTypeAndConstraint ::
      FragmentMetadata ->
      Either RenderingIssue (PostgresqlType, NullabilityConstraint)
    toTypeAndConstraint = \case
      UnknownFragment fragment ->
        Left $ FragmentMetadataContainsUnknownFragment fragment
      ColumnFragment fragment ->
        case fragment.columnType of
          PgUnknown unknownType ->
            Left $ FragmentMetadataContainsUnknownType unknownType
          _otherwise ->
            Right (fragment.columnType, fragment.columnNullConstraint)
      ExpressionFragment fragment ->
        case fragment.expressionType of
          PgUnknown unknownType ->
            Left $ FragmentMetadataContainsUnknownType unknownType
          _otherwise ->
            Right (fragment.expressionType, fragment.expressionNullConstraint)
...
renderingIssueToHuman ::
  RenderingIssue ->
  Text
renderingIssueToHuman = \case
  FragmentMetadataContainsUnknownFragment unknownFragment ->
    "Encountered some SQL that could not be handled: " <> unknownFragment
  FragmentMetadataContainsUnknownType unknownType ->
    "Encountered a type that could not be handled: " <> unknownType
